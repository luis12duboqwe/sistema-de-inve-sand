...
export function exportOrdersToCSV(orders: OrderWithItems[], profileName?: string) {
  try {
    ...
    throw new Error(`Failed to export orders: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
...
export function exportOrderDetailsToCSV(order: OrderWithItems) {
  try {
    ...
    throw new Error(`Failed to export order details: ${error instanceof Error ? error.message : 'Unknown error'}`)
  }
}
