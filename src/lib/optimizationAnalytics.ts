...
export function analyzeInventory(
  ...
  products.filter(p => p.activo).forEach(product => {
    ...
    if (totalSold > 20 && avgMonthlyDemand > 0) { 0.25)
...
e()
  const sixtyDaysAgo = new Date()go.getDate() - 60)
  sixtyDaysAgo.setDate(sixtyDaysAgo.getDate() - 60)
stomer => {
  customerMap.forEach(customer => {
    if (customer.orders.length >= 3 && customer.totalSpent > 5000) {
      insights.push({',
        type: 'high_value',mer.name,
        customerName: customer.name,e,
        customerPhone: customer.phone,
        metric: 'Total Gastado',
        value: customer.totalSpent,s.length} compras y $${customer.totalSpent} en valor total. Top 10% de la base.`,
...
ustomer.orders.length >= 3 && customer.lastOrder < sixtyDaysAgo) {
    if (customer.orders.length >= 3 && customer.lastOrder < sixtyDaysAgo) {)
      ...
      insights.push({
        ...
        value: daysSinceLastOrder, (${customer.orders.length} compras) inactivo ${daysSinceLastOrder} días. Alto riesgo de pérdida.`,
 ...
 2 && customer.lastOrder >= sixtyDaysAgo) {
    if (customer.orders.length === 2 && customer.lastOrder >= sixtyDaysAgo) {
      insights.push({
        type: 'growth_opportunity',
        customerName: customer.name,
        customerPhone: customer.phone,
        metric: 'Compras',ders.length,
        value: customer.orders.length,interés. Está en fase de conversión a recurrente.`,
  ...
  const avgOrderValue = orders.length > 0 ders.length 
    ? orders.reduce((sum, o) => sum + o.total, 0) / orders.length 
    : 0
 (avgOrderValue > 0) {
  if (avgOrderValue > 0) {rs = Array.from(customerMap.values())
...
export function analyzeProductMix(
  ...
  const thirtyDaysAgo = new Date()Ago.getDate() - 30)
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30)
p => p.activo).map(product => {
  const productPerformance = products.filter(p => p.activo).map(product => {
    ...
    const totalRevenue = productOrders.reduce((sum, order) => {
      const item = order.items.find(i => i.product_id === product.id) 0))
      return sum + ((item?.precio_unitario || 0) * (item?.cantidad || 0))
    }, 0)
ders.reduce((sum, order) => {
    ...
    const profit = totalRevenue - (totalQuantity * product.precio * 0.6)
alRevenue, quantity: totalQuantity, profit }
    return { product, revenue: totalRevenue, quantity: totalQuantity, profit }
  })
  const topByRevenue = productPerformance
  const topByRevenue = productPerformance
    .filter(p => p.revenue > 0)evenue)
    .sort((a, b) => b.revenue - a.revenue)
    .slice(0, 3)
{
  if (topByRevenue.length > 0) {ue.reduce((sum, p) => sum + p.revenue, 0)
    const totalRevenue = topByRevenue.reduce((sum, p) => sum + p.revenue, 0)
    insights.push({eller',
      type: 'best_seller',enue.map(p => p.product.id),
      productIds: topByRevenue.map(p => p.product.id),mbre),
      productNames: topByRevenue.map(p => p.product.nombre),
      metric: 'Ingresos últimos 30 días',
      value: totalRevenue,oductos generan $${totalRevenue} (${topByRevenue.map(p => p.product.nombre).join(', ')}). Concentrar estrategia aquí.`,
  ...
  const poorPerformers = productPerformance< 0)
    .filter(p => p.revenue > 0 && p.profit < 0)
    .sort((a, b) => a.profit - b.profit)
    .slice(0, 3)
  if (poorPerformers.length > 0) {
  if (poorPerformers.length > 0) {
    insights.push({erformer',
      type: 'poor_performer',ers.map(p => p.product.id),
      productIds: poorPerformers.map(p => p.product.id),mbre),
      productNames: poorPerformers.map(p => p.product.nombre),
      metric: 'Productos con pérdida',
      value: poorPerformers.length,length} productos vendiendo por debajo del costo. Están destruyendo margen.`,
  ...
  productPerformance.forEach(p => {
    ...
    if (!brandGroups.has(brand)) {ducts: [], revenue: 0 })
      brandGroups.set(brand, { products: [], revenue: 0 })
    }randGroups.get(brand)!.products.push(p.product)
  ...
  if (topBrand && topBrand[1].revenue > 0) {
    insights.push({ry_trend',
      type: 'category_trend',.products.map(p => p.id),
      productIds: topBrand[1].products.map(p => p.id),mbre),
      productNames: topBrand[1].products.map(p => p.nombre),
      metric: 'Marca líder',enue,
      value: topBrand[1].revenue,domina con $${topBrand[1].revenue} en ingresos. ${topBrand[1].products.length} SKUs activos.`,
      reasoning: `${topBrand[0]} domina con $${topBrand[1].revenue} en ingresos. ${topBrand[1].products.length} SKUs activos.`,a`,
...
export function analyzeOperations(
  products: ProductWithStock[],
  orders: OrderWithItems[]
): OperationalInsight[] {onalInsight[] = []
  const insights: OperationalInsight[] = []
  const ordersByDay = new Map<number, number>()
  const ordersByDay = new Map<number, number>()
  orders.forEach(order => {der.created_at).getDay()
    const day = new Date(order.created_at).getDay()) + 1)
  ...
  const peakDay = Array.from(ordersByDay.entries())
    .sort((a, b) => b[1] - a[1])[0]
  if (peakDay) {
  if (peakDay) { ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado']
    const days = ['Domingo', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado']
    insights.push({',
      type: 'timing', de demanda',
      area: 'Patrones de demanda',
      metric: 'Día pico',[0]],
      value: days[peakDay[0]],ay[0]]} concentra ${peakDay[1]} órdenes, ${Math.round((peakDay[1] / orders.length) * 100)}% del total.`,
  ...
  if (pendingOrders.length > 5) {
    insights.push({s',
      type: 'process',ento de órdenes',
      area: 'Procesamiento de órdenes',
      metric: 'Órdenes pendientes',
      value: pendingOrders.length,length} órdenes en estado pendiente pueden indicar cuellos de botella en fulfillment.`,
  ...
  if (lowStockItems > 3) {
    insights.push({tion',
      type: 'automation',nventario',
      area: 'Gestión de inventario',e quiebre',
      metric: 'Productos en riesgo de quiebre',
      value: lowStockItems,kItems} productos bajo umbral crítico. Reorden manual es ineficiente y riesgoso.`,
      reasoning: `${lowStockItems} productos bajo umbral crítico. Reorden manual es ineficiente y riesgoso.`,automatizada.`
      action: `Implementar alertas automáticas por WhatsApp cuando stock < 5. Crear lista de reorden semanal automatizada.`
    })
  }
  const avgOrderValue = orders.length > 0 
  const avgOrderValue = orders.length > 0 tal, 0) / orders.length 
    ? orders.reduce((sum, o) => sum + o.total, 0) / orders.length 
    : 0
  if (avgOrderValue && avgOrderValue < 500) {
  if (avgOrderValue && avgOrderValue < 500) {
    insights.push({ce',
      type: 'resource',ransacción',
      area: 'Valor de transacción',
      metric: 'Ticket promedio',rderValue)}`,
      value: `$${Math.round(avgOrderValue)}`,th.round(avgOrderValue)}). Pequeño incremento tiene gran impacto acumulado.`,
  ...
  if (cancelledOrders.length > 0) {lledOrders.length / orders.length) * 100
    const cancellationRate = (cancelledOrders.length / orders.length) * 100
    if (cancellationRate > 10) {
      insights.push({s',
        type: 'process', de ventas',
