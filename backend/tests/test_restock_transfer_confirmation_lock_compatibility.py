...
def test_missing_destination_stock_is_serialized_with_inventory_count_approval(
    db_session: Session,
) -> None:
    suffix = uuid4().hex
    product = Product(
        sku=f"TRANSFER-COUNT-MISSING-{suffix}",
        nombre=f"Producto Transfer Count Missing {suffix}",
        categoria="accesorio",
        marca="Marca QA",
        modelo=f"Modelo-{suffix[:8]}",
        condicion="nuevo",
    ...
    destination = Location(
        nombre=f"Destino Transfer Count Missing {suffix}",
        tipo="tienda",
        activo=True,
    )
    source = Location(
        nombre=f"Origen Transfer Count Missing {suffix}",
        tipo="tienda",
        activo=True,
    )
    db_session.add_all([product, destination, source])
    ...
    count = PhysicalInventoryCount(
        ...
        status="draft",
        counted_by="qa",
        notes="missing-stock creation race",
    )
    db_session.add_all([transfer, count])
    db_session.flush()
    transfer_id = int(transfer.id)
    ...
    confirmation_product_lock_attempted = Event()
    count_product_lock_sql: list[str] = []
    confirmation_product_lock_sql: list[str] = []
    ...
    def before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
        ...
        if (
            ...
            confirmation_product_lock_attempted.set()
    ...
    def after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
        ...
        if (
            ...
            confirmation_product_lock_attempted.wait(timeout=5)
            return
        ...
        if (
            ...
            with state_lock:
                confirmation_product_lock_sql.append(normalized)
        ...
        try:
            ...
            try:
                result = multistore_control.approve_inventory_count(
                    count_id,
                    InventoryCountApproveRequest(notes="approved first"),
                    session,
                    _user(),
                )
                return 200, result.status
            except Exception as exc:  # noqa: BLE001 - regression captures DB conflicts
                session.rollback()
                return int(getattr(exc, "status_code", 500)), repr(exc)
        finally:
            with state_lock:
    ...
    try:
        with ThreadPoolExecutor(max_workers=2) as pool:
            futures = [pool.submit(run_count_approval), pool.submit(run_confirmation)]
            results = [future.result(timeout=20) for future in futures]
    finally:
    ...
    assert confirmation_product_lock_sql
    assert all("FOR NO KEY UPDATE" in sql for sql in count_product_lock_sql)
    assert all("FOR NO KEY UPDATE" in sql for sql in confirmation_product_lock_sql)
    ...
    assert destination_rows[0].cantidad_disponible == 5
    ...
    assert (
        ...
        .filter(
            ...
            StockHistory.location_id == destination_id,
