...
# Keep the same explicit safety boundary used by canonical bank/location
# identities. Python's Unicode database can lag newer Format additions, so these
# ranges supplement ``unicodedata.category``.
...
def _assert_safe_unicode(value: str) -> None:
    if any(
        ...
        raise ValueError(
            "El nombre del proveedor contiene caracteres Unicode no permitidos"
        )
