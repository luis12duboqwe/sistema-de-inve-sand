...
WARRANTY_ACTION = "warranty_exchange"
...
def list_returns(
    ...
    """Lista el historial de devoluciones/cambios por garantía."""
...
def create_return(
    ...
    """Procesa únicamente cambios por garantía de una venta finalizada.
    ...
    if order.estado not in FINAL_RETURNABLE_ORDER_STATUSES:
        if order.estado == "cancelada":
            detail = "No se pueden procesar garantías de órdenes canceladas"
        else:
            detail = (
                "La garantía solo puede procesarse después de completar la venta. "
                "Para una orden pendiente o por entregar, edítela o cancélela en lugar de procesar una garantía."
            )
    ...
    for item in return_data.items:
        ...
        if action != WARRANTY_ACTION:
            raise HTTPException(
                status_code=400,
                detail="La política comercial solo permite cambio por garantía; no se realizan reembolsos ni crédito en tienda.",
            )
    ...
    for prepared in prepared_items:
        if prepared.action != WARRANTY_ACTION:
            raise HTTPException(
                status_code=400,
                detail="La política comercial solo permite cambio por garantía.",
            )
        ...
        stock_manager.process_return_stock(
            ...
            notes=f"Garantía Orden #{order.id}: {prepared.condition}",
            user_id=user_name,
        )
    ...
    try:
        ...
        db.rollback()
        logger.exception("Error al procesar garantía de la orden %s", return_data.order_id)
        raise HTTPException(
            status_code=500,
            detail="Error interno al procesar la garantía. Intente nuevamente o contacte al administrador."
        )
